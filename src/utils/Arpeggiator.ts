...
export default class Arpeggiator {
  ...
  public static genRandomConfig(defaultValues?: { patternSize?: number; maxSubdiv?: number; numAttacks?: number; contourSize?: number }): {
    ...
    const contourSize = defaultValues?.contourSize || Math.max(6, Math.floor(Math.random() * numAttacks * 2 + numAttacks));
  ...
  public static arpeggiate(
    ...
    const timeOffset = Math.floor(sortedChords[0].onset / patternDuration) * patternDuration;
